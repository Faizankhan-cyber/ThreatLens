      ...
      <div className="surface rounded-xl p-5 sm:p-6">
        ...
        {tab === 'SCREENSHOT' ? (
          ...
          >
            ...
            />
            <button type="submit" disabled={scanning} className="btn-primary sm:whitespace-nowrap">
        ...
        {tab !== 'SCREENSHOT' && tab !== 'QR' && tab !== 'URL' && (
          <div className="mt-2 text-right text-xs text-slate-600">
            {input.length} / 2000 characters
          </div>
        )}

        {/* Scan button (hidden for URL — button is inline with input) */}
        {tab !== 'URL' && (
          <div className="mt-4">
            <button onClick={handleScan} disabled={scanning} className="btn-primary w-full sm:w-auto">
              {scanning ? <Loader2 className="h-4 w-4 animate-spin" /> : <ScanLine className="h-4 w-4" />}
              {scanning ? 'Analyzing...' : `Analyze ${tab.charAt(0) + tab.slice(1).toLowerCase()}`}
            </button>
          </div>
        )}
      </div>
